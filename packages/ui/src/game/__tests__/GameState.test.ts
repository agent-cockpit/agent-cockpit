...
describe('GameState', () => {
  ...
  it("gameState.player initial value has x: 192 (2*96), y: 480 (5*96), direction: 'south'", () => {
    expect(gameState.player).toEqual({ x: 2 * 96, y: 5 * 96, direction: 'south' })
  })
  ...
  it('WORLD_W equals 96 * 20 (1920) and WORLD_H equals 96 * 15 (1440)', () => {
    expect(WORLD_W).toBe(1920)
    expect(WORLD_H).toBe(1440)
  })
})
